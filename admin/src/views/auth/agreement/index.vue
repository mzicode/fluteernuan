<template>
  <div class="agreement-page">
    <div class="agreement-card">
      ...
      <p class="hint">未确认协议前，服务端会阻止管理后台业务接口。</p>
    </div>
  </div>
</template>
...
<script setup lang="ts">
  ...
  async function load() {
    loading.value = true
    try {
      agreement.value = await getAdminAgreement()
    } catch (error) {
  ...
  async function submit() {
    ...
    try {
      await acceptAdminAgreement({ version: agreement.value.version, content_hash: agreement.value.content_hash })
